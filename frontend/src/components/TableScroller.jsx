      ...
      {state.overflow && (
        <>
          <button
            ...
            className={`hidden sm:flex items-center justify-center absolute left-1 top-1/2 -translate-y-1/2 z-20 w-9 h-9 rounded-full bg-white/95 shadow-lg border border-slate-200 text-[#C1401E] transition-opacity hover:bg-[#C1401E] hover:text-white ${state.canL ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
            <ChevronLeft className="w-5 h-5" />
          </button>
          <button
            type="button" onClick={() => scrollBy(320)} aria-label="Desplazar a la derecha"
            data-testid={`${testid}-right`}
            className={`hidden sm:flex items-center justify-center absolute right-1 top-1/2 -translate-y-1/2 z-20 w-9 h-9 rounded-full bg-white/95 shadow-lg border border-slate-200 text-[#C1401E] transition-opacity hover:bg-[#C1401E] hover:text-white ${state.canR ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
